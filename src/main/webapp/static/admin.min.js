 ...
 * blog.afnf.net
 */
;afnfblog.delayedTids={};afnfblog.delayedExec=function(b,a){var c=afnfblog.delayedTids[b];if(c){clearTimeout(c)}afnfblog.delayedTids[b]=setTimeout(b,a)};afnfblog.update_preview=function(){var a=function(){var d=$(".textarea_entry");if(d.size()>0){var c=d.val();var b=marked(c);$(".entry_content").html(b);$("#contentHtml").val(b);afnfblog.highlight()}};if(afnfblog.isMobile){afnfblog.delayedExec(a,2000)}else{afnfblog.delayedExec(a,300)}};afnfblog.admin_entry_init=function(a){$(a).removeClass("btn-default").addClass("btn-primary");var b=$(".textarea_entry");if(b.size()>0){$(".entry_edit_form_div").on("keyup",".textarea_entry",afnfblog.update_preview);$(".entry_edit_parent_div").on("click",".preview_width",function(){var d=$(this);$(".preview_width").removeClass("btn-primary");d.addClass("btn-primary");var c=d.text();$(".preview_container").css("width",c)});afnfblog.update_preview()}};afnfblog.admin_comment_init=function(){$(".btn_comments").removeClass("btn-default").addClass("btn-primary");$(".comments_container").on("click",".shortened_comment",function(a){$(this).removeClass("shortened_comment")});$(".comments_container").on("click",".stateRadio",function(a){$(this).parent("form").submit()})};afnfblog.ajaxSuccess=function(b,a){$(".entry_edit_form_div #id").val(a.id);var c=b.find(".ajaxret");c.text(t("blog.update_ok"));setTimeout(function(){c.text("")},3000)};